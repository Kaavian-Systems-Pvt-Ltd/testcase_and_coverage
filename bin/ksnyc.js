...
const { cliWrapper, suppressEPIPE } = require('../../nyc/lib/commands/helpers')
const foreground = require('foreground-child')
const resolveFrom = require('resolve-from')
const NYC = require('../../nyc/index.js')
...
async function main () {
  ...
  if (!argv.instrument) argv.instrumenter = '../../nyc/lib/instrumenters/noop'
  else argv.instrumenter = '../../nyc/lib/instrumenters/istanbul'
  ...
  if (!argv.useSpawnWrap) {
    const requireModules = [
      require.resolve('../../nyc/lib/register-env.js'),
      ...nyc.require.map(mod => resolveFrom.silent(nyc.cwd, mod) || mod)
    ]
    const preloadList = require('node-preload')
    preloadList.push(
      ...requireModules,
      require.resolve('../../nyc/lib/wrap.js')
    )
